...
class ExpressionTreePuzzle(Puzzle):
    ...
    def is_solved(self) -> bool:
        ...
        for variable in self.variables:
            if str(self.variables[variable]) not in '123456789':
                return False
        ...
        for variable in var:
            if var[variable] == 0:
                ...
                while i <= 9:
                    dic1 = var.copy()
                    dic1[variable] = i
                    # should we also create a copy of puz?
                    exp_t = self._tree.copy()
                    tar = self.target
                    puz = ExpressionTreePuzzle(exp_t, tar)
                    puz.variables = dic1
                    result.append(puz)
                    i += 1
        return result
    ...
    def fail_fast(self) -> bool:
        ...
        for key in self.variables:
            if self.variables[key] == 0:
                ...
                min_dic[key] = self.variables[key]
                max_dic[key] = self.variables[key]
        if self._tree.eval(min_dic) > self.target or \
                self._tree.eval(max_dic) < self.target:
            return True
        return False
