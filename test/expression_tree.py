...
class ExprTree:
    ...
    def __str__(self) -> str:
        ...
        if self.is_empty():
            ...
            seperate = ' ' + str(self._root) + ' '
            return f'({result[0]} {self._root} {seperate.join(result[1:])})'
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        True
        """
        if str(self._root) not in '123456789' and self._root not in OPERATORS:
            lookup[self._root] = 0
        for subtree in self._subtrees:
